...
import { getUsuarioNombre, getUsuarioRol, getProyectos, getMateriales, getAlertasProyectos } from "../services/api";
...
// Estilo compartido para los 4 cards del grid: anula cualquier grid-column de clases CSS
const CARD_STYLE = {
    background: "#fffdf8",
    border: "1px solid #e6dfd2",
    borderRadius: "26px",
    padding: "22px",
    gridColumn: "auto",   // evita que .notifications-card fuerce grid-column: 2/3
};
...
function Home() {
    ...
    const cargarDatos = async () => {
        try {
            ...
            const conteo = { pendiente: 0, en_curso: 0, finalizado: 0, cancelado: 0 };
            proyectos.forEach((p) => {
    ...
    const formatearEstadoVisual = (estado) => {
        switch (estado) {
            case "pendiente":  return { clase: "pending",     texto: "Pendiente" };
            case "en_curso":   return { clase: "in-progress", texto: "En progreso" };
            case "finalizado": return { clase: "done",        texto: "Completado" };
            case "cancelado":  return { clase: "cancelled",   texto: "Cancelado" };
            default:           return { clase: "",            texto: estado };
        }
    };

    const iconoEstado = (estado) => {
        switch (estado) {
            case "pendiente":  return { icono: "⚡", clase: "yellow" };
            case "en_curso":   return { icono: "▦",  clase: "" };
            case "finalizado": return { icono: "✓",  clase: "green" };
            case "cancelado":  return { icono: "✕",  clase: "red" };
    ...
    return (
        <AppLayout>
            ...
            <section className="welcome-card home-hero-card">
                <div className="welcome-left">
                    ...
                    <div>
                        ...
                        <p>Aquí puedes revisar rápidamente los indicadores principales de ServiElec Manager.</p>
                    </div>
                </div>
                <button onClick={() => navigate("/proyectos")}>Ver proyectos <span>→</span></button>
            </section>
            ...
            <section className="stats-grid">
                <div className="stat-card">
                    <div className="stat-top">
                        <div><span>Proyectos activos</span><h3>{cargando ? "..." : statsProyectos.activos}</h3></div>
                ...
                <div className="stat-card">
                    <div className="stat-top">
                        <div><span>Inventario total</span><h3>{cargando ? "..." : statsInventario.total}</h3></div>
                        <div className="stat-icon">◈</div>
                    </div>
                ...
                <div className="stat-card">
                    <div className="stat-top">
                        <div><span>Stock crítico</span><h3>{cargando ? "..." : statsInventario.criticos}</h3></div>
                        <div className="stat-icon warning-icon">▤</div>
                    </div>
                    <p className={statsInventario.criticos > 0 ? "negative" : ""}>
                        {statsInventario.criticos > 0 ? "materiales en alerta" : "todo en orden"}
                    </p>
                </div>
                <div className="stat-card">
                    <div className="stat-top">
                        <div><span>Total proyectos</span><h3>{cargando ? "..." : statsProyectos.total}</h3></div>
                        <div className="stat-icon">☷</div>
                    </div>
                    <p>en el sistema</p>
                </div>
            ...
            {/*
              ── Grid 2×2 ─────────────────────────────────────────
              Fila 1: [Gráfico]           [Proyectos recientes]
              Fila 2: [Alertas inventario][Proyectos próximos]

              NOTA: se usa gridColumn:"auto" en cada hijo para anular
              la regla CSS ".notifications-card { grid-column: 2/3 }"
              que existe en index.css y que forzaría los widgets a col 2.
            */}
            <section
                style={{
                    display: "grid",
            ...
            >
                {/* Celda [1,1] — Gráfico de proyectos */}
                <div style={CARD_STYLE}>
                    <div className="card-title-row">
                        <h2>Gráfico de proyectos</h2>
                    </div>
                    {cargando ? (
                        <p style={{ textAlign: "center", padding: "2rem" }}>Cargando gráfico...</p>
                    ) : datosGrafico.length === 0 ? (
                        <p style={{ textAlign: "center", padding: "2rem" }}>Aún no hay proyectos para graficar.</p>
                    ) : (
                    ...
                    {cargando ? (
                        ...
                        proyectosRecientes.map((proyecto) => {
                            ...
                            return (
                                ...
                                >
                                    <div className={`project-icon ${icono.clase}`}>{icono.icono}</div>
                                    <div>
                                        <strong>{proyecto.nombre_proyecto}</strong>
                                        <p>Cliente: {proyecto.nombre_cliente}</p>
                                    </div>
                                    <span className={`status ${estadoVisual.clase}`}>{estadoVisual.texto}</span>
                                </div>
                            );
                        })
                    )}
                ...
                {/* Celda [2,1] — Alertas de inventario */}
                <div style={CARD_STYLE}>
                    <div className="card-title-row">
                        <h2>Alertas de inventario</h2>
                        <button onClick={() => navigate("/notificaciones")}>Ver todas</button>
                    </div>
                    {cargando ? (
                        ...
                        alertas.slice(0, 3).map((material) => (
                            <div
                                key={material.id_material}
                                className={`notification-item ${material.stock_actual === 0 ? "danger" : "warning"}`}
                            >
                                <div className="notification-icon">{material.stock_actual === 0 ? "!" : "⌛"}</div>
                                <div>
                                    <strong>
                                        {material.stock_actual === 0 ? "Sin stock: " : "Stock bajo: "}
                                        {material.nombre_material}
                                    </strong>
                                    <span>Disponible: {material.stock_actual} | Critico: {material.stock_critico}</span>
                                </div>
                            </div>
                        ))
                    )}
                ...
                <div style={CARD_STYLE}>
                    ...
                    {cargando ? (
                        <p>Cargando...</p>
                    ) : alertasProyectos.slice(0, 3).length === 0 ? (
                        <div className="notification-item info">
                            <div className="notification-icon">i</div>
                            <div>
                        ...
                        alertasProyectos.slice(0, 3).map((a) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div>
                                        ...
                                        <span>{a.nombre_cliente} · {diasTexto}</span>
                                    </div>
                                </div>
                            );
                        })
