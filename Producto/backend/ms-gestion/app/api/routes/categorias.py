...
def obtener(categoria_id: str, db: Session = Depends(get_db), _=Depends(get_current_user)):
...
def eliminar(categoria_id: str, db: Session = Depends(get_db), _=Depends(require_admin)):
